...
<br><br>\
<form>\
<span style=\"color:#881111\"><h1>SET VALUE FOR DIMMER-MASTER</h1></span>\
<span style=\"color:#ff4000\"><h3>VALUE DATA FORM 0-TO-100.</h3></span>\
<table border=\"3\" bgcolor=\"#ffffff\">\
<td><input type=\"integer-allowed\" size=\"15\" name=\"ip1\" ip1=\"\" /></td>\
<td><input type=\"submit\" style='height:30px;width:100px;background:#F4A460' value=\"SAVE\" / >\
</table>\
</form>\
</body>\
</html>\
...
void saveDIMVALUE(){
 unsigned char i=11;
//SAVE_VALUE_DIM=(((unsigned long)buf[0])<<24)|(((unsigned long)buf[1])<<16)|(((unsigned long)buf[2])<<8)|(((unsigned long)buf[3]));
//FLASH_Write_Word(0x08007F80,SAVE_VALUE_DIM); //AddressDIMVALUE = 0x08007F80
//SAVE_VALUE_DIM=(((unsigned long)buf[4])<<24)|(((unsigned long)buf[5])<<16)|(((unsigned long)buf[6])<<8)|(((unsigned long)buf[7]));
//FLASH_Write_Word(0x08007F90,SAVE_VALUE_DIM); //AddressDIMVALUE = 0x08007F90
//SAVE_VALUE_DIM=(((unsigned long)buf[8])<<24)|(((unsigned long)buf[9])<<16)|(((unsigned long)buf[10])<<8)|(((unsigned long)buf[11]));
//FLASH_Write_Word(0x08007FA0,SAVE_VALUE_DIM); //AddressDIMVALUE = 0x08007FA0
//SAVE_VALUE_DIM=(((unsigned long)buf[12])<<24)|(((unsigned long)buf[13])<<16)|(((unsigned long)buf[14])<<8)|(((unsigned long)buf[15]));
//FLASH_Write_Word(0x08007FB0,SAVE_VALUE_DIM); //AddressDIMVALUE = 0x08007FB0

//SAVE_VALUE_DIM=(((unsigned long)buf[i+3])<<24)|(((unsigned long)buf[i+2])<<16)|(((unsigned long)buf[i+1])<<8)|(((unsigned long)buf[i]));
//FLASH_Write_Word(0x08007F80,SAVE_VALUE_DIM); //AddressDIMVALUE = 0x08007F80
//SAVE_VALUE_DIM=(((unsigned long)buf[i+7])<<24)|(((unsigned long)buf[i+6])<<16)|(((unsigned long)buf[i+5])<<8)|(((unsigned long)buf[i+4]));
//FLASH_Write_Word(0x08007F90,SAVE_VALUE_DIM); //AddressDIMVALUE = 0x08007F90
//SAVE_VALUE_DIM=(((unsigned long)buf[i+11])<<24)|(((unsigned long)buf[i+10])<<16)|(((unsigned long)buf[i+9])<<8)|(((unsigned long)buf[i+8]));
//FLASH_Write_Word(0x08007FA0,SAVE_VALUE_DIM); //AddressDIMVALUE = 0x08007FA0
//SAVE_VALUE_DIM=(((unsigned long)buf[i+15])<<24)|(((unsigned long)buf[i+14])<<16)|(((unsigned long)buf[i+13])<<8)|(((unsigned long)buf[i+12]));
//FLASH_Write_Word(0x08007FB0,SAVE_VALUE_DIM); //AddressDIMVALUE = 0x08007FB0
...
void saveIP(unsigned char buf[]){
    ...
    while(i<39){
        if(buf[i]=='i'&&buf[i+1]=='p'){
            for(j=0;j<4;j++){
                    if(buf[i+4]>=48&&buf[i+4]<=57&&(buf[i+5]<48||buf[i+5]>57)){       // 48= "0"; 57="9"
...
unsigned int SPI_Ethernet_UserTCP(unsigned char *remoteHost, unsigned int remotePort, unsigned int localPort, unsigned int reqLength, TEthPktFlags *flags){
    ...
    if(getRequest[5]=='t'){                    // GET /t8
        if(isdigit(getRequest[6])){            // getRequest[6] ="8"
            index_page=getRequest[6]-'0';
            if(index_page==Page_Setting){        // 192.168.1.123/t8?ip1=xxx.xxx.xx.xxx
                _locate=10;                       // getRequest[10] ="1"
                for(h=0;h<4;h++){
                    locate=get_point(getRequest,_locate,25);      // lan dau tien h=0  locate=15
            ...
            if(index_page==Page_Service){          // 192.168.1.123/t1?ip1=99  when typing"99"
              saveDIMVALUE();
            }
        }
    }
    ...
    switch (index_page) {
    case Page_Service:
        //copyBuf(getRequest,buf,40);   // WHY ? ORG= 40
        //RECEIVE 16 first bytes  "GET /t1 HTTP/1.1"
        //saveDIMVALUE();
        if(getRequest[5] == 'f'){
            len += putConstString("r");
            len += putString(dyna);
    ...
    case Page_Device_Extant:
        ...
        break;
    case Page_Setting:
    // khi vua moi bam vao trang Setting thi no da chay cac lenh nay
        copyBuf(getRequest,buf,40);   // WHY ? = 40
        // getRequest[0] -> getRequest[116]
        // GET /t1 HTTP/1.1
        // /r/n
        // Host:192.168.1.123
        // /r/n
    ...
    if(len == 0){
        ...
        case Page_Login:
            ...
            break;
        case Page_Service:
            // len += putConstString(indexPage);     DIM_VALUE PAGE
...
// [W7=0][W6][W5][W4][W3][W2][W1][W0]
    ...
    if(ETHERNET_BUTTON_DEVICE_0==1){
                             ...
                             if(ETHERNET_BUTTON_DIM0==1){VALUE4CHANNEL=40;} //  ASCII "1"
                             if(ETHERNET_BUTTON_DIM0==2){VALUE4CHANNEL=49;} //  ASCII "9"
